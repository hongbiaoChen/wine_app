	...
	function cancelMask(){
		...
		$('.cancelOrder').on('touchstart',function(){
			...
			$('.select').on('touchstart',function(){
				...
				if(choseSrc==="/resources/img/shopcar_images/tuoyuan.png"){
					$(this).attr('src','/resources/img/shopcar_images/xuanze@2x.png').parent().siblings().children('.select').attr('src','/resources/img/shopcar_images/tuoyuan.png');
				}
			})
			//点击取消按钮关闭遮罩层
